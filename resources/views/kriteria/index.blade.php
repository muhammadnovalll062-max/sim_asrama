...
<style>
    ...
    .badge-secondary{background:#fce7f3;color:#9d174d}
...
<div class="page-header">
    <div class="page-title">Data Kriteria Profile Matching</div>
...
<div class="card">
    <table>
        <thead>
            <tr><th>No</th><th>Kode</th><th>Nama Kriteria</th><th>Nilai Ideal</th><th>Jenis</th><th>Aksi</th></tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $k->nilai_ideal }}</td>
                <td><span class="badge badge-{{ strtolower($k->jenis) }}">{{ $k->jenis }}</span></td>
                <td>
                    <a href="{{ route('kriteria.edit', $k->id_kriteria) }}" class="btn btn-warning">Edit</a>
                    <form method="POST" action="{{ route('kriteria.destroy', $k->id_kriteria) }}" style="display:inline">
                        @csrf @method('DELETE')
                        <button class="btn btn-danger" onclick="return confirm('Hapus kriteria ini?')">Hapus</button>
                    </form>
                </td>
            </tr>
            @empty
            <tr><td colspan="6" style="text-align:center;color:#6b7280;padding:2rem">Belum ada kriteria.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
